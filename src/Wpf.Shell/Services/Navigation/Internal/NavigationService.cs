...
namespace Wpf.Shell.Services.Navigation.Internal
{
    ...
    internal sealed class NavigationService : ObservableObject, INavigationService
    {
            ...
            IViewModelFactory factory)
        {
            ...
            // called by ShellBuilder after the host is built — past the window in which resolving
            // a root would recurse into the facade's own construction graph.
        }

        public void InitializeTabs(IReadOnlyList<TabRegistration> registrations)
        {
            if (_initialized)
            ...
            // One pass in list order; resolve each root via the factory and wrap it in a
            // TabHistory. The factory is the single seam to DI — direct IServiceProvider access is
            // forbidden in the navigation framework. Roots are singletons, so the factory returns
            // the canonical instances.
            //
            // Tab order comes from the builder's list, not from container enumeration. The
            // duplicate check below is a backstop; ShellBuilder.AddTab already rejects duplicates
